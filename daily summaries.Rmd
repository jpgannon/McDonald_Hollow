...
wetdrywide <- submerged1%>%
  pivot_wider(names_from = Name, values_from = inWater, date)

wetdrylong <- wetdrywide%>%
  pivot_longer(!date, names_to = "Name", values_to = "inWater")
wetdry <- left_join(wetdrylong, locations, by = c("Name" = "Name"))


write.csv(wetdry,"G:/My Drive/Independent Study Fall 2020/McDonald_Hollow//wetdry.csv")
