...
subMS8_AirL %>% filter(Type == "Stream", Stream == "MS", between(DateTime, as_datetime("2020-08-15 13:30:00"), as_datetime("2020-09-01 13:30:00")))%>%
...
subMS8L %>% filter(Name %in% sites, between(DateTime, as_datetime("2020-08-15 13:30:00"), as_datetime("2020-09-01 13:30:00")))%>%
ggplot(aes(DateTime, TempFsub, color = Name))+
  geom_line()+
  labs(title = "Mainstem in-stream sensors", x = "Time", y = "Temperature")
```
...
```{r}
#plot smaller groups
tempdata%>% filter(Name == c("MS1", "MS2", "TRIB1"))%>%
...
```{r}
#plot smaller groups
tempdata%>% filter(Name == c("MS14", "MS15"))%>%
ggplot(aes(DateTime, TempCor_F, color = Name))+
  geom_line()+
...
```{r}
#plot smaller groups
tempdata%>% filter(Name == c("MS18", "MS19", "MS19_Air"))%>%
ggplot(aes(DateTime, TempCor_F, color = Name))+
  geom_line()+
